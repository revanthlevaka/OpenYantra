...
def detect_version(path: str) -> str:
    ...
    try:
        ...
        if "🛡️ Security Log" in sheets or "📊 Stats" in sheets:
            # Check for v2.9+ additions
            try:
                df = pd.read_excel(path, sheet_name="🔓 Open Loops",
                                   engine="odf", header=0)
                if "📒 Agrasandhanī" in sheets and "🔒 Quarantine" in sheets:
                    # Could be 2.9, 2.10, 2.11, 2.12 -- check Security Log
                    if "🛡️ Security Log" in sheets:
                        return "2.9+"
                return "2.4+"
            except Exception:
                return "2.4+"
...
def build_migration_plan(from_version: str, to_version: str = "2.8") -> list[dict]:
    ...
    # → v2.9+: Add Security Log sheet if missing
    if from_version not in ("2.9+",):
        steps.append({
            "step": "add_sheet",
            "description": "Add 🛡️ Security Log sheet (v2.4+)",
            "sheet": "🛡️ Security Log",
            "columns": ["Timestamp", "Agent", "Sheet", "Threat Level",
                        "Threat Type", "Threats", "Status"],
        })
    ...
    # → v2.12: nothing structural -- version string update only
    steps.append({
        "step": "update_index",
        "description": "Update INDEX sheet to reflect v2.12 schema",
    })
