...
class OpenYantra:
    ...
    def request_write(self, req: WriteRequest) -> dict:
        """All writes go through Chitragupta. VidyaKosha syncs after commit."""
        receipt = self._ledger.submit(req)
        if receipt.get("status") == "written" and self._vidyakosha is not None:
            try:
                # v2.1: incremental update if available, else full sync
    ...
    def search(self, query: str, top_k: int = 5, sheets: list = None,
               snapshot_mode: str = None, hybrid_alpha: float = 0.7) -> list[dict]:
        ...
        return self._vidyakosha.query(
            text=query, agent_name=self.agent_name, top_k=top_k,
...
def run_bootstrap_interview(path: str, agent_name: str = "Chitragupta"):
    """
    Terminal Bootstrap Interview — cold start via conversation.
    Populates 12 sheets through natural questions instead of empty forms.
    """
    oy = OpenYantra(path, agent_name=agent_name)
    if oy.path.exists():
        print(f"[OpenYantra] Chitrapat already exists at {oy.path}")
        print("[OpenYantra] Run `yantra ui` to review and edit it.")
        return oy

    print("\n" + "="*60)
    print("  OpenYantra — Chitragupta Puja")
    print("  The Sacred Memory Machine v2.1")
    print("  Inspired by the Hindu God of Data")
    print("="*60)
    print("\nNamaste. I am Chitragupta, your memory keeper.")
    print("Let me learn about you so I can serve you well.")
    print("(Press Enter to skip any question)\n")

    def ask(question: str) -> str:
        try: return input(f"  {question}: ").strip()
        except (EOFError, KeyboardInterrupt): return ""

    # Identity
    name       = ask("What is your name")
    occupation = ask("What do you do (occupation / role)")
    location   = ask("Where are you based (city, country)")
    language   = ask("Primary language (default: English)")
    hours      = ask("Working hours (e.g. 10am-7pm IST)")

    oy.bootstrap(user_name=name, occupation=occupation, location=location)
    if language:
        oy.update_identity("Primary Language", language)
    if hours:
        oy.update_identity("Working Hours", hours)

    # Goals
    print("\n  Tell me about your goals:")
    for goal_type in ["Your most important long-term goal",
                      "A short-term goal you're working on right now"]:
        goal = ask(goal_type)
        if goal:
            oy.request_write(WriteRequest(
                agent_name, SHEET_GOALS, "add",
                {"Goal": goal,
                 "Type": "Long-term" if "long" in goal_type else "Short-term",
                 "Priority": "High", "Status": "Active"},
                confidence="High", source="User-stated", importance=9,
            ))

    # Active projects
    print("\n  Tell me about your active projects:")
    for i in range(1, 3):
        project = ask(f"Active project {i} name (or skip)")
        if not project: break
        domain   = ask(f"  Domain for '{project}' (Work/Creative/Personal)")
        next_step = ask(f"  Next step for '{project}'")
        oy.add_project(project, domain=domain or "Work",
                       next_step=next_step, importance=8)

    # People
    print("\n  Tell me about important people in your work/life:")
    for i in range(1, 3):
        person = ask(f"Important person {i} name (or skip)")
        if not person: break
        role = ask(f"  Relationship with {person}")
        oy.add_person(person, relationship=role or "Contact", importance=7)

    # Preferences
    print("\n  A few preferences:")
    comm_style = ask("Communication style (direct/formal/casual)")
    tools_pref = ask("Preferred tools or software")
    if comm_style:
        oy.request_write(WriteRequest(
            agent_name, SHEET_PREFERENCES, "add",
            {"Category": "Communication", "Preference": comm_style,
             "Strength": "Strong"},
    ...
    if tools_pref:
        oy.request_write(WriteRequest(
            agent_name, SHEET_PREFERENCES, "add",
            {"Category": "Tools", "Preference": tools_pref, "Strength": "Strong"},
            confidence="High", source="User-stated", importance=6,
        ))

    print("\n" + "="*60)
    print(f"  ✓ Chitrapat created at {oy.path}")
    print(f"  ✓ {len(oy._read_sheet(SHEET_PROJECTS))} projects recorded")
    print(f"  ✓ {len(oy._read_sheet(SHEET_GOALS))} goals recorded")
    print(f"  ✓ {len(oy._read_sheet(SHEET_PEOPLE))} people recorded")
    print(f"\n  Run `yantra ui` to open your memory dashboard.")
    print(f"  Or: libreoffice '{oy.path}'")
    print("="*60 + "\n")
    return oy
