"""
yantra_ui.py — OpenYantra Browser Dashboard v2.9.1
...
app = FastAPI(title="OpenYantra", version="2.9.1", docs_url=None, redoc_url=None)
_oy = None
...
<header>
  <div>
    ...
    <small>Sacred Memory Machine · v2.9.1 · Chitragupta-secured</small>
  </div>
...
async function loadToday(){
  ...
  const ctxBtn = `<div style="margin-bottom:14px;display:flex;gap:10px;align-items:center">
    ...
    document.getElementById('today').innerHTML='<div class="cards">'+cards+'</div>'+ctxBtn+body;
}
...
def main():
    parser = argparse.ArgumentParser(description="OpenYantra Dashboard v2.9.1")
